...
class ReportGenerator:
    ...
    # 日报提示词前缀
    DAILY_PROMPT_PREFIX = """【最高优先级：智能纪要与逐字稿】
输入中可能包含「飞书文档」部分，其中标题包含"智能纪要"、"会议纪要"的文档是**最重要的信息来源**，请：
1. **优先阅读**所有智能纪要、会议纪要文档，这些文档包含会议的完整逐字稿和详细记录
2. **深度挖掘**智能纪要中的每一句话，不要遗漏任何有价值的内容
3. **重点提取**：会议中的精彩发言、关键决策、讨论过程、行动事项
4. **智能纪要的权重是最高的**，比普通聊天记录更重要
5. 如果智能纪要中有逐字稿内容，请**详细记录**每个人的发言

【次高优先级：飞书私聊与群聊】
输入中「飞书会话」部分是**第二重要的信息来源**，请：
1. **重点关注私聊内容**：一对一的私聊往往包含深入的辅导、讨论、决策
2. **仔细阅读群聊**：群聊中的讨论、问题解答、协作都是重要内容
3. **不要过滤任何有价值的对话**：即使看起来琐碎的对话也可能包含重要信息
4. **重点提取**：你辅导下属的对话、你与他人的讨论、你解决问题的过程
...
【重要：知识沉淀说明】
请从今日工作内容中深度挖掘并提取以下知识沉淀内容（**全面提取，不要遗漏，即使内容较多也要完整记录**）：

【重要：挖掘深度要求】
- **第一优先级**：仔细阅读**所有**智能纪要、会议纪要文档，逐字阅读逐字稿内容
- **第二优先级**：仔细阅读**所有**飞书私聊（一对一对话），这是深度辅导和讨论的富矿
- **第三优先级**：仔细阅读**所有**飞书群聊、Claude 会话记录
- 重点关注：你辅导下属的对话、你与团队的讨论、会议中的精彩发言、你解决问题的完整过程
- 不要只看表面，要挖掘对话背后的思考逻辑、决策过程、方法论
- **智能纪要中的内容权重最高**，优先从智能纪要中提取金句、决策逻辑、会议精彩瞬间

1. 今日金句（**3-5句，宁缺毋滥但要有深度**）：
   - 从今日工作中提炼最有启发性的话
   - 重点来源：
     * 你在会议中的精彩发言或总结
     * 你辅导下属时说的有价值的话
...
2. 辅导与沟通（**这是核心！要详细记录**）：
   - 记录今天你**如何辅导下属、指导他人、解决团队问题**的完整过程
   - 每个案例包含：
     * 场景：对方遇到了什么问题
     * 你的辅导方式：你说了什么、做了什么、用了什么方法
     * 对方的反馈：对方如何回应、问题是否解决
     * 你的反思：这个辅导过程中有什么可以改进的地方
   - 重点关注：
     * 你如何教下属解决技术问题
     * 你如何协调团队矛盾
     * 你如何给下属分配任务并跟进
...
3. 决策逻辑（**详细记录每个决策的完整过程**）：
   - 记录今天做过的**所有重要决策**，包括业务决策、技术决策、团队管理决策
   - 每个决策用"背景-讨论过程-选项-结论"四段式呈现：
     * 背景：为什么需要做这个决策，问题是什么
     * 讨论过程：大家是怎么讨论的，有哪些不同观点
     * 选项：具体考虑过哪些方案，每个方案的优缺点
     * 结论：最终选择了什么，核心理由是什么，是谁拍板的
   - 即使是小决策，只要有讨论过程就要记录

4. SOP沉淀（**从解决问题的过程中提炼可复用的方法论**）：
   - 如果今天解决了某个问题，把解决过程总结成标准化操作步骤
   - 包含：
     * 适用场景：什么情况下用这个SOP
     * 操作步骤：1/2/3... 详细的步骤
     * 注意事项：关键风险、避坑点、容易出错的地方
     * 实际案例：今天是如何应用这个SOP的
   - 重点关注：
     * 排查问题的流程
     * 跨部门协作的流程
...
5. 会议精彩瞬间（**记录会议中最有价值的部分**）：
   - 记录今天参加的会议中，最有价值的讨论、最精彩的发言、最重要的共识
   - 每个记录包含：
     * 会议名称
     * 精彩瞬间：谁说了什么、大家的反应如何
     * 价值点：为什么这个瞬间重要，带来了什么启发
...
## 七、今日金句
> [3-5句有深度的话，来自今日工作中的感悟、会议中的精彩发言、辅导下属时说的话]
> 来源: [相关来源和具体场景]

## 八、辅导与沟通
- [辅导案例1名称]
  - 场景: 对方遇到了什么问题
  - 我的辅导方式: 我说了什么、做了什么、用了什么方法
  - 对方的反馈: 对方如何回应、问题是否解决
  - 我的反思: 这个辅导过程中有什么可以改进的地方
  - 来源: [相关来源]
...
## 七、今日金句
> 好的设计是把复杂的事情变简单，而不是把简单的事情变复杂。
> 来源: Claude 项目会话（日报模板优化讨论）

> 用效果竞价的方式拿量，同时还能全额累计品牌框架返点，一鱼两吃。
> 来源: 飞书私聊 会话 oc_b06054418（4月投放重点方向）

## 八、辅导与沟通
- 辅导严建淮解决狂刀千年游戏包上传问题
  - 场景: 严建淮上传游戏包时遇到安全域名配置问题
  - 我的辅导方式: 让林贤达先给严建淮开通权限，提醒检查安全域名配置，上传后提醒测试完整性
  - 对方的反馈: 严建淮配合度高，问题顺利解决
  - 我的反思: 跨团队协助时，权限开通和配置检查是关键前置步骤
  - 来源: 飞书私聊 会话 oc_a6158e50f

## 九、决策逻辑
- 角色语音Demo方案选型
  - 背景: 游戏端需要NPC对话语音能力，需兼顾性能和成本
  - 讨论过程: 大家讨论了火山端到端方案的便捷性，以及阿里qwen+cosyvoice方案的灵活性
  - 选项: ① 火山端到端语音对话方案；② 阿里qwen-plus-character做文本生成+CosyVoice做语音生成
  - 结论: 两套方案并行做Demo，对比效果和成本后再选型
  - 来源: 飞书群 射雕AI语音-贪玩内部沟通

## 十、SOP沉淀
- Seedream可信资产使用SOP
  - 适用场景: 使用Seedance2.0生成视频时引用素材，需要避免合规风险
  - 操作步骤: ① 使用Seedream5.0生成原始图片；② 直接引用图片URL或上传本地文件到Seedance2.0；③ 生成视频直接使用
  - 注意事项: 原始图片不可使用其他大模型编辑，编辑后的图片失去可信属性，无法直接引用
  - 实际案例: 今天确认Seedream5.0生成的原始图无需上传可信资产库可直接引用
  - 来源: 飞书私聊 钱建锋

## 十一、会议精彩瞬间
- 短剧美术中台功能规划评审会
  - 精彩瞬间: 讨论暗水印方案时，大家快速对齐了两套方案的优劣势和适用场景
  - 价值点: 体现了团队高效决策的能力——先列方案，再比优劣，最后定优先级
  - 来源: 飞书群 短剧-美术中台功能规划讨论

## 十二、行业资讯
- 4月短视频投放新政策更新
  - 核心要点: ① 红果首页信息流、Topview硬广全新上线，适合泛娱乐、下沉人群的小游戏/手游拓客，性价比高；② 星图火热内测"明星AI素材"，低门槛、高产能，跑量能力突出，可快速提升点击率；③ AD原生互动、声量宝等竞价产品正式纳入品牌竞价消耗，可同时享受效果拿量和品牌返点
  - 对我的影响/启发: 后续短剧投放可尝试红果渠道，使用明星AI素材降低素材生产成本，同时采用内容营销产品兼顾效果和品牌返点，提升ROI
  - 来源: 飞书私聊 会话 oc_b06054418
"""
